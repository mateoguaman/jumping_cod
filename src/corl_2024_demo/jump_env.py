...
def create_sim(sim_conf):
  ...
  if sim_conf.show_gui:
    ...
    viewer = None
  # return gym, sim, viewer  ## TODO: Check if this is correct
...
class JumpEnv:
  def __init__(self,
    ...
    # self._robot._motor_torque_delay_steps = self._config.get('motor_torque_delay_steps', 0)
    # self._robot._camera_config = self._config.get('camera_config', None)
    # self._robot._randomize_com = self._config.get('randomize_com', False)
  ...
  def reset_idx(self, env_ids) -> torch.Tensor:
    ...
    if env_ids.shape[0] > 0:
      ...
      if not self._use_real_robot and self._config.terrain.curriculum:
        if self._config.terrain.type != "plane":
  ...
  def _is_done(self):
    is_unsafe = torch.logical_or(self._robot.projected_gravity[:, 2] < 0.1,
                                 self._robot.base_position[:, 2] < 0.1)  ## TODO: Might have to be -0.1

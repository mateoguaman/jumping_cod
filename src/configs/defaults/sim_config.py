...
"""Default simulation config. Use this for demo purposes, above for training"""
...
def get_config(use_gpu=True,
  ...
  if not use_real_robot:
    ...
    sim_params.physx.num_threads = 10
    sim_params.physx.solver_type = 1  # 0: pgs, 1: tgs
    sim_params.physx.num_position_iterations = 8 #4
    sim_params.physx.num_velocity_iterations = 2 #0
    if use_penetrating_contact:
      sim_params.physx.contact_offset = 0.
      sim_params.physx.rest_offset = -0.01
    else:
      sim_params.physx.contact_offset = 0.02 # 0.01
      sim_params.physx.rest_offset = 0.001 #0.
    sim_params.physx.bounce_threshold_velocity = 0.2 #0.5  #0.5 [m/s]
    sim_params.physx.max_depenetration_velocity = 5.0 #1.0
    sim_params.physx.max_gpu_contact_pairs = 2**24  #2**24 needed for 8000+ envs
    sim_params.physx.default_buffer_size_multiplier = 5
    sim_params.physx.contact_collection = gymapi.ContactCollection(
        2)  # 0: never, 1: last sub-step, 2: all sub-steps (default=2)
    config.sim_params = sim_params
